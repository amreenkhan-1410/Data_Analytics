...
# comparing the total layoffs based on company, industry, country, date , stage
 ...
 select min(`date`),max(`date`) from layoffs_staging3;
  ...
  select `date` , sum(total_laid_off) from layoffs_staging3 group by `date` order by 1 desc;
    select year(`date`) , sum(total_laid_off) from layoffs_staging3 group by year(`date`) order by 1 desc;
  select stage , sum(total_laid_off) from layoffs_staging3 group by stage order by 1 desc;
  
  select substring(`date`,1,7) as `month`,sum(total_laid_off) from layoffs_staging3 
  where substring(`date`,1,7) is not null group by `month` order by 1 asc;
  
  # rolling total adds the data of layoffs year by year and shows which year have high increase in the layoffs
  with rolling_total as 
  (
  select substring(`date`,1,7) as `month`,sum(total_laid_off) as total_off from layoffs_staging3 
  where substring(`date`,1,7) is not null group by `month` order by 1 asc
  )
   select `month`,total_off,sum(total_off) over(order by `month`) as rolling_total 
   from rolling_total; 
   
   # categorizing the layoffs year by year based on the company 
   ...
   # using ctes to rank the companies based on layoffs 
